...
render(){
    ...
    this.setState({ userThings: userThings.filter( userThing => userThing.id !== userThingToDestroy.id), error: ''});
    };
    ...
    try {
        ...
        this.setState({ things: things.filter( thing => thing.id !== thingToDestroy.id), error: ''});
    }
    catch(ex){
        this.setState({ error: ex.response.data.message})
    }
